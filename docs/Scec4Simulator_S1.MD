...
flowchart TD
    ...
    D --> E[BpmnSimulationReadinessValidator]
    E --> F[ExecutionGraphCompiler]
    B --> G[VersionResolver]
    B --> H[ArrivalProcessGenerator]
    B --> I[CaseAttributeGenerator]
    B --> J[GatewayRuleEvaluator]
    B --> K[ResourceManager]
    F --> L[SimPyRuntimeEngine]
    G --> L
    H --> L
    I --> L
    J --> L
    K --> L
    L --> M[SimulationEventCollector]
    M --> N[XesSimulationWriter]
    M --> O[SummaryWriter]
    M --> P[GeneratedDataConfigWriter]
```
...
sequenceDiagram
    ...
    participant Validator as Readiness Validator
    ...
    UC->>Validator: validate supported BPMN subset
    Validator-->>UC: warnings/errors
...
src/
  ...
  domain/
    ...
    services/
      simulation/
        bpmn_simulation_readiness_validator.py
  ...
  adapters/
    export/
      ...
      generated_data_config_writer.py
tools/
...
- Версія процесу задається в конфізі явно як `version_id + bpmn_path (+ active_from для активації)`.
- Якщо в конфізі одночасно описано кілька версій, то `case_start_time` використовується лише для вибору активної BPMN-версії на момент старту кейсу.
- Кейс після старту **залишається** у своїй обраній версії до завершення.
- Лог один, суцільний, з overlap між версіями.

Важливо: selection активної версії в симуляторі і train/val/test split у pipeline це різні задачі. Їх не треба змішувати в одному дизайні.
...
#### Що означає `arrival_process.type: poisson`

Для `rate_per_hour = λ`:

- у середньому стартує `λ` кейсів на годину;
- інтервали між стартами мають розподіл `Exponential(λ)`;
- якщо горизонт симуляції дорівнює `T` годин, то очікувана кількість кейсів `E[N] = λ * T`;
- фактична кількість кейсів флуктуює і має розподіл `Poisson(λ * T)`.

Приклад:

- `rate_per_hour: 4.0`
- горизонт симуляції `10` діб = `240` годин

Тоді:

- очікуваний обсяг логу `960` кейсів;
- фактична кількість у конкретному прогоні буде коливатись навколо цього значення;
- стандартне відхилення становитиме `sqrt(960)`.

Отже обсяг логу контролюється двома осями:

1. інтенсивністю `rate_per_hour`;
2. часовим горизонтом `simulation.start_time -> simulation.end_time`.

Щоб контроль був жорсткішим, Stage 1 доцільно одразу проєктувати з optional обмеженням:
...
У такому режимі генерація завершується по першій умові:

- досягнуто `end_time`;
- або досягнуто `max_cases`.
...
Подія `assign` не є XES de-facto стандартом, але для дослідження черг і ресурсів вона корисна.

Тому для Stage 1 рекомендовано:

- `assign` configurable, default `true` для human tasks;
- `start` обов'язкова;
- `complete` обов'язкова.

Якщо `assign` увімкнено:

- `assign` пишеться коли задача призначена конкретному виконавцю і поставлена в його чергу;
- `start` пишеться коли ресурс реально взяв задачу;
- `complete` пишеться по завершенні duration.
...
Для automatic task мінімальний життєвий цикл має бути:

- `start`
- `complete`

`assign` для automatic task не є обов'язковим і за замовчуванням у Stage 1 його краще не писати.

Рекомендований default:

- human task: `assign/start/complete`
- automatic task: `start/complete`
...
output:
  ...
  generated_data_config_path: "configs/data/generated_loan_v1_v2.yaml"
  overwrite: true
...
### 6.4 Generated data config

Окрім датасета Stage 1 має генерувати готовий data-config у `configs/data`, щоб лог можна було одразу підключити до pipeline.
...
data:
  source:
    type: xes
    path: outputs/simulation/loan_v1_v2.xes
  process_name: loan_process
  mapping:
    xes_adapter:
      ...
      start_transitions: ["assign", "start"]
```

Це артефакт симулятора, а не hand-written експериментальний конфіг.
...
| `time:timestamp` | event timestamp | Обов'язково |
  | `lifecycle:transition` | `start` / `complete`, optionally `assign` | Обов'язково |
...
Порядок tie-break має залежати від того, чи увімкнено `assign`.

Рекомендований порядок:

- якщо `assign` присутній: `assign -> start -> complete`
- якщо `assign` відсутній: `start -> complete`

Інакше ingestion може давати нестабільні active-set snapshots.

## 8. BPMN readiness validation before generation

Перед запуском симуляції потрібен окремий `BpmnSimulationReadinessValidator`.

### 8.1 Warning-only елементи, які не впливають на flow
...
Такі елементи не повинні блокувати генерацію. Їх можна ігнорувати з warning у summary/report.

### 8.2 Error елементи, які впливають на flow, але не підтримуються
...
Такі елементи повинні блокувати генерацію для відповідної BPMN-версії.

### 8.3 Поведінка validator

- збирає `warnings`;
- збирає `errors`;
- при наявності `errors` симуляція не стартує;
- пише короткий readiness report у лог і в summary JSON.

## 9. Stage 1 implementation plan
...
3. Додати `BpmnSimulationReadinessValidator`.
4. Додати `ExecutionGraphCompiler`, який з `ProcessStructureDTO` будує runtime graph:
   - node registry;
   - outgoing/incoming flow indices;
   - gateway metadata;
   - join counters.
5. Реалізувати `VersionResolver` по `active_from`.
6. Реалізувати `CaseAttributeGenerator`.
7. Реалізувати `GatewayRuleEvaluator`.
8. Реалізувати `ResourceSelector` і просту модель worker queues.
9. Реалізувати `SimPyRuntimeEngine`.
10. Реалізувати `SimulationEventCollector` + `XesSimulationWriter`.
11. Додати `GeneratedDataConfigWriter`.
12. Додати CLI tool `simulate-versioned-log`.
13. Додати integration smoke test: `config -> simulate -> xes_adapter.read`.
...
- `VersionResolver` повертає активну BPMN-версію по `case_start_time`;
- `BpmnSimulationReadinessValidator` коректно ділить unsupported elements на warnings/errors;
...
- generated `configs/data/*.yaml` відкривається поточним loader без ручних правок.
...
## 11. Межа між симулятором і training pipeline

Симулятор не повинен проєктуватись навколо `train_ratio` або `temporal_cutoff`.

Його задача:

- згенерувати коректний суцільний лог;
- зберегти коректні timestamps;
- зберегти `concept:version`;
- згенерувати data-config.

Рішення про те, як саме відсікати train/val/test, є окремим concern основного pipeline.

## 12. Потрібна чи ні зміна основного pipeline для time boundaries
...
### 12.2 Мінімальна рекомендація

Після підтвердження Stage 1 додати в main pipeline техборг на `split_strategy: temporal_cutoff`:
...
Також можна окремо передбачити режим комбінування з `%`-split, якщо це буде потрібно в основному скрипті:

- або відсікаємо по `%`;
- або по часу;
- або по тому критерію, який спрацює першим.
...
- `--data-config-out` як optional override для `generated_data_config_path`
- `--seed` як optional override

## 14. Bottom line for Stage 1
