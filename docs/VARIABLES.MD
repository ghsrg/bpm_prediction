...
* **$E_{struct}$** | `structural_edge_index` — Множина дозволених (нормативних) ребер процесу згідно з регламентом $\kappa$.
...
### 7.1 Drift Window Policy Variables (MVP1)
* **$w$** | `window_size` — Розмір вікна оцінювання дрейфу (у кількості трас).
* **$\Delta$** | `window_stride` — Крок зсуву вікна між ітераціями.
* **$o$** | `window_overlap` — Перекриття між сусідніми вікнами, де \(\Delta = w-o\).
* **$t_{start}^{(k)}$** | `window_start_ts` — Початкова часова мітка \(k\)-го вікна.
* **$t_{end}^{(k)}$** | `window_end_ts` — Кінцева часова мітка \(k\)-го вікна.

### 7.2 Feature Policy Variables (MVP1)
* **$z_{min}$** | `z_clip_min` — Нижня межа clipping після Z-score.
* **$z_{max}$** | `z_clip_max` — Верхня межа clipping після Z-score.
* **$\mathcal{D}_{work}$** | `work_days` — Набір робочих днів тижня для `time2vec_work_hours`.
* **$h_{start}$** | `work_start_hour` — Початок робочого інтервалу.
* **$h_{end}$** | `work_end_hour` — Кінець робочого інтервалу.
...
* **$OOS$** | `oos_rate` (Out of Structure / Out of Sequence) — Частка прогнозів моделі $\hat{y}$, які не існують у нормативному графі ($ (e_{last}, \hat{y}) \notin E_{struct} $). Використовується як міра галюцинацій моделі.

### 12. Data Structures Mapping
- $e_i$ : `EventRecord` (Подія на індексі $i$).
- $\sigma$ : `RawTrace` (Повна історія кейсу).
- $\sigma_{\leq k}$ : `PrefixSlice` (Префікс траси до кроку $k$, де $e_{k+1}$ є цільовим $y$).
- $G_{obs}$ : `InstanceGraph` (Граф, побудований на основі $\sigma_{\leq k}$).
- $T$ : `GraphTensorContract` (Тензорне представлення графа $G_{obs}$ та структурної маски).
- $G_{struct}^{(k)}$ : `ProcessStructureDTO` (Контейнер нормативного графа для поточної версії $\kappa$, містить `allowed_edges`).

### 13. Трекінг для експериментів (MLflow)
* Для режимів `inference` та `evaluate` логувати в трекер (MLflow):
  ...
  - `semaphore_mode`
  - `test_oos_rate`
  - `window_oos_rate` (для графіка дрейфу)
* Мінімальна частота логування: кожний batch або інший атомарний крок інференсу.

### Векторизація та розмірності (MVP1)
- $|V_{act}|$ (`num_activities`): Розмір словника унікальних активностей (включно з `<UNK>`).
- $|V_{res}|$ (`num_resources`): Розмір словника унікальних ресурсів (включно з `<UNK>`).
- $F_{in}$ (`input_dim`): Початкова розмірність вектора ознак вузла $x_v \in \mathbb{R}^{F_{in}}$. 
  Для MVP1 розраховується як сума One-Hot векторів та неперервних ознак часу: 
  $F_{in} = |V_{act}| + |V_{res}| + 3$.
- $X_V$ (`x`): Матриця ознак усіх вузлів графа розмірністю $[N_{nodes}, F_{in}]$.
