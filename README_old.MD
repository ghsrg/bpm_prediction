...
Архітектура `bpm_prediction` будується відповідно до принципів:
...
- уніфікований інтерфейс моделей, приймає динамічний набор даних та фітч вказаних в конфігах,
- окремі конфігурації для кожного бізнес-процесу,
- повний seed-контроль,
- підтримка best/last моделей,
- відновлення навчання,
- логування часу та візуалізація % виконання,
...
Graph Construction Layer відповідає за **повну формалізацію бізнес-процесу у вигляді графів**, які надалі обробляються Feature Engineering Layer та Model Layer.

Цей шар складається з окремих незалежних будівників графів та одного інтеграційного компонента.

---

#### **2.1. Instance Graph (IG)**  
Граф, побудований виключно з журналів подій (Camunda / XES):

- **вузли:** реальні події інстансу процесу  
- **ребра:** фактичні переходи між подіями (`SEQUENCE_COUNTER_`, порядковість, часові різниці)  
- **атрибути:** тривалість, ресурс, overdue, payload, timestamps  

Instance Graph описує **реальний перебіг виконання процесу**, без урахування BPMN-моделі.

---

#### **2.2. BPMN Graph (BG)**  
Граф, побудований зі структури BPMN-моделі:

- tasks, gateways, events  
- sequence flows  
- структури lane → role → resource  
- нормативні (дозволені) переходи  

BPMN Graph — це **нормативна модель процесу**, незалежно від того, як він працює в реальності.

---

#### **2.3. POKG (Process Organizational Knowledge Graph)**  
Опційний граф знань, що включає:

- ресурси → ролі → підрозділи  
- SLA та policy-зв'язки  
- семантичні ознаки (документні embeddings)  
- історичні патерни переходів  
- міжпроцесні залежності  

POKG надає **макрорівневий організаційний контекст**, який може підвищувати точність моделей.

---

#### **2.4. Fusion Graph — єдине об’єднане представлення**

Fusion Graph Builder формує **універсальний граф**, який об’єднує IG + BPMN + POKG.  
Це НЕ просто “склеювання” структур, а їх **нормалізоване та узгоджене об’єднання**.

Fusion Graph містить:

- `node_features`  
- `edge_features`  
- `edge_index`  
- `global_features`  
- `allowed_transition_mask` (матриця допустимих переходів із BPMN)  
- `timestamps` та їхні похідні  
- `bpmn_structural_features`  
- `doc_features` / embeddings  
- `activity_labels`, `global_node_ids`, `prefix_length`  

Fusion Graph Builder НЕ створює тензори PyTorch — він генерує **універсальний формат**, який буде конвертований у PyG Data на наступному шарі (Feature Engineering).

---

#### **2.5. Уніфікація формату графів**

Всі графи на виході з Graph Construction Layer приводяться до єдиного формату:

- внутрішнє представлення → `networkx.DiGraph()`  
- об’єднане представлення → `FusionGraphData` (власна структура)  
- готове до моделі представлення → PyTorch Geometric `Data` (створюється Feature Builder-ом)

---

### **Результат роботи Graph Construction Layer**

На виході формується **Fusion Graph** — стандартизоване, об’єднане, нормалізоване та універсальне графове представлення процесу, яке єдиним форматом подається у Feature Engineering Layer та всі GNN-моделі.
...
Гіперпараметри отримуються з файлу конфіга або динамічно розраховуються
...
4.1. Data Acquisition
   - import_events_pipeline
   - import_bpmn_pipeline

4.2. Graph Construction
   - build_instance_graph_pipeline
   - build_bpmn_graph_pipeline
   - build_fusion_graph_pipeline

4.3. Feature Engineering
   - feature_extraction_pipeline
   - prefix_generation_pipeline

4.4. Model Training
   - train_gnn_pipeline
   - train_predictive_pipeline

4.5. Evaluation & Analytics
   - evaluate_pipeline
   - prefix_evaluation_pipeline
   - comparison_pipeline
   - experiment_pipeline
