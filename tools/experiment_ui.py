...
class ExperimentUI:
    ...
    def _humanize_path_ua(path: str) -> str:
        explicit = {
            ...
            "training.dynamic_candidate_contract_enabled": "Dynamic candidate contract",
            "training.candidate_contract_mode": "Контракт кандидатів",
            "training.candidate_batch_topology_policy": "Політика batch топології",
            "model.topology_conditioning_mode": "Режим topology conditioning",
            "model.impulse_activation_enabled": "Impulse activation routing",
            "model.impulse_scale_init": "Початковий масштаб impulse",
            "model.impulse_scale_max": "Максимальний масштаб impulse",
            "model.impulse_gnn_layers": "Кількість impulse GNN layers",
            "model.impulse_residual_h0": "Impulse residual h0",
            "model.impulse_state_channels": "Канали prefix-state impulse",
    ...
    def _hint_text_for_path(field_path: str) -> str:
        exact: Dict[str, str] = {
            ...
            "training.dynamic_candidate_contract_enabled": "true вмикає model.forward_candidate() для EOPKGTopologyConditioned compatibility path.",
            "training.candidate_contract_mode": "Контракт prediction/loss: fixed_label = legacy, fixed_projection = dynamic candidates projected to labels, candidate_id = true candidate-level mode.",
            "training.candidate_batch_topology_policy": "Для candidate_id батчі мають бути однорідні за process_version_idx + stats_snapshot_version_idx.",
            "model.topology_conditioning_mode": "static_candidates = попередній candidate scorer; impulse_activation_routing додає prefix-state impulse у topology-native candidate scoring.",
            "model.impulse_activation_enabled": "true примусово вмикає impulse_activation_routing для EOPKGTopologyConditioned.",
            "model.impulse_scale_init": "Початковий bounded scale для normalized impulse перед structural GNN.",
            "model.impulse_scale_max": "Верхня межа scale, щоб impulse не домінував над base candidate state.",
            "model.impulse_gnn_layers": "Глибина diffusion для impulse routing. Початковий експеримент: 1.",
            "model.impulse_residual_h0": "Додає h0 назад після structural message passing.",
            "model.impulse_state_channels": "Логічні канали struct_prefix_state_x, які використовуються як execution pointer/state.",
